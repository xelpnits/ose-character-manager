...
import { Character, INITIAL_CHARACTER, Item, ItemCategory, AbilityScore } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedChars) {
      try {
        ...
        const migratedChars = parsedChars.map((c: any) => ({
            ...c,
            // 1. Container Types
            containers: c.containers.map((cont: any) => ({
                ...cont,
                type: cont.type || (cont.isFixed ? 'equipped' : 'carried')
            })),
            // 2. Temp Stats
            tempHp: c.tempHp || 0,
            acModifier: c.acModifier || 0,
            // 3. Modifiers Objects
            abilityModifiers: c.abilityModifiers || {
                [AbilityScore.STR]: 0,
                [AbilityScore.INT]: 0,
                [AbilityScore.WIS]: 0,
                [AbilityScore.DEX]: 0,
    ...
    reader.onload = (e) => {
        try {
            ...
            if (json.characters && Array.isArray(json.characters)) {
                // Apply migration to imported data too
                const migratedChars = json.characters.map((c: any) => ({
                    ...c,
                    containers: c.containers.map((cont: any) => ({
                        ...cont,
                        type: cont.type || (cont.isFixed ? 'equipped' : 'carried')
                    ...
                    acModifier: c.acModifier || 0,
                    abilityModifiers: c.abilityModifiers || {
                        [AbilityScore.STR]: 0, [AbilityScore.INT]: 0, [AbilityScore.WIS]: 0,
                        [AbilityScore.DEX]: 0, [AbilityScore.CON]: 0, [AbilityScore.CHA]: 0,
                    },
                    saveModifiers: c.saveModifiers || {
                        death: 0, wands: 0, paralysis: 0, breath: 0, spells: 0
                    }
  ...
  const handleNewCharacter = useCallback(() => {
    const newChar: Character = { 
        ...
        name: 'New Character',
        containers: [
            { id: crypto.randomUUID(), name: 'Worn / Belt', type: 'equipped', items: [], isFixed: true },
  ...
  // Handle Global Add
  const handleGlobalAddItem = useCallback((charId: string, containerId: string, item: Item) => {
     if (charId === BANK_ID) {
