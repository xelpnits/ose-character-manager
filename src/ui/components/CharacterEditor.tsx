...
import { ArrowLeft, Shield, Heart, Skull, Zap, Trophy, CloudLightning, Calculator, Activity, PlusCircle, MinusCircle } from 'lucide-react';
            ...
            <div className="xl:col-span-3 space-y-6">
                ...
                <div className="glass-panel rounded-2xl p-5">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xs font-bold uppercase tracking-widest text-indigo-400">Abilities</h2>
                                     ...
                                     <div className="flex flex-col items-end">
                                        <input 
                                            ...
                                            className={`w-8 h-6 text-center text-[10px] bg-slate-950 border rounded focus:ring-0 focus:border-cyan-500
                                                ${(character.saveModifiers?.[saveName as keyof typeof character.saveModifiers] || 0) !== 0 ? 'border-cyan-500/50 text-cyan-400' : 'border-slate-800 text-slate-600'}
                                            `}
                                        />
                                     </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="glass-panel rounded-2xl p-5 relative overflow-hidden group">
                        ...
                        <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between relative z-10">
                             ...
                             <div className="flex items-center gap-2">
                                <button onClick={() => onUpdate({...character, tempHp: Math.max(0, (character.tempHp || 0) - 1)})} className="text-slate-500 hover:text-white"><MinusCircle className="w-4 h-4" /></button>
                                <input 
                                    type="number"
                                    value={character.tempHp || 0}
                                    onChange={(e) => onUpdate({...character, tempHp: Math.max(0, parseInt(e.target.value) || 0)})}
                                ...
                                />
                                <button onClick={() => onUpdate({...character, tempHp: (character.tempHp || 0) + 1})} className="text-slate-500 hover:text-white"><PlusCircle className="w-4 h-4" /></button>
                             </div>
                        </div>
                        ...
                        <div className="mt-4 pt-3 border-t border-white/5 relative z-10">
                             ...
                             <div className="flex justify-between gap-1">
                                ...
                                <div className="flex gap-1">
                                    <QuickModButton val={1} onClick={() => onUpdate({...character, acModifier: (character.acModifier || 0) + 1})} />
                                    <QuickModButton val={2} onClick={() => onUpdate({...character, acModifier: (character.acModifier || 0) + 2})} />
                                    <QuickModButton val={-1} onClick={() => onUpdate({...character, acModifier: (character.acModifier || 0) - 1})} />
                                </div>
                             </div>
                        </div>
