        ...
        <div className="animate-fade-in max-w-4xl mx-auto">
          <div className="glass-panel rounded-3xl p-1 bg-gradient-to-br from-amber-950/20 to-slate-950/50 border border-amber-500/20 shadow-2xl">
            <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                <h2 className="text-2xl font-serif text-amber-400 flex items-center gap-3">
                  <Sparkles className="w-6 h-6" /> Party Loot
                </h2>
                <p className="text-slate-400 text-sm">
                  Unclaimed treasure worth <span className="text-amber-300 font-bold">{totalLootValue} gp</span>
                </p>
              </div>
              {newLootItems.length > 0 && (
                <button
                  onClick={() => setShowClaimConfirm(true)}
                  className="px-6 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-bold shadow-lg transition-all hover:scale-105"
                >
                  Claim All Loot
                </button>
              )}
            ...
            <div className="p-6">
              {newLootItems.length === 0 ? (
                <div className="text-center py-16">
                  <div className="bg-slate-800/50 p-6 rounded-full mb-6 ring-1 ring-white/10 inline-block">
                    <Sparkles className="w-12 h-12 text-slate-600" />
                  </div>
                  <h3 className="text-xl font-serif text-slate-400 mb-2">No Unclaimed Loot</h3>
                  <p className="text-slate-500 text-sm">All treasure has been claimed or there's nothing valuable yet.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {newLootItems.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-white/5 hover:border-amber-500/30 transition-colors"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-400">
                            Carried by {item.carrierName} • {item.containerName}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-amber-400 font-bold">{(item.goldValue || 0) * item.count} gp</div>
                        {item.count > 1 && (
